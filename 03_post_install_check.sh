...
print_summary() {
  ...
  if [[ "$ENABLE_CXX_RUNNER_PROXY_FINAL" == "1" ]]; then
    echo "C++ runner proxy:          enabled (${CXX_RUNNER_PORT})"
  else
    echo "C++ runner proxy:          disabled"
  fi
  if [[ "$ENABLE_PY_RUNNER_PROXY_FINAL" == "1" ]]; then
    echo "Python runner proxy:       enabled (${PY_RUNNER_PORT})"
  else
    echo "Python runner proxy:       disabled"
  fi
