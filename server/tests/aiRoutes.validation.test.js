...
test('response evaluation includes prior attempts in the prompt when history exists', async () => {
  ...
  try {
    ...
    assert.match(userMessage, /core answer plus reasonable reasoning/i);
    assert.match(userMessage, /weaken the requirements/i);
...
test('response evaluation returns the exact fallback for an out-of-domain question', async () => {
  const originalFetch = global.fetch;
  global.fetch = async (input, init) => {
    const url = typeof input === 'string' ? input : input?.url || '';
            ...
            choices: [
              {
                ...
                message: {
                  ...
                  content: JSON.stringify({
                    inDomain: false,
                    feedback: 'Something unrelated and overlong.',
                  }),
                },
                finish_reason: 'stop',
              },
